...
function addTask(mock){
    ...
    creationDate.innerText = `Data criada: ${inputs[0].value.split("-").reverse().join("-")}`;
    if(!mock){
        dueDate.innerText = `Data limite: ${inputs[1].value.split("-").reverse().join("-")}`;
    }else dueDate.innerText = "Data limite: desconhecida.";
